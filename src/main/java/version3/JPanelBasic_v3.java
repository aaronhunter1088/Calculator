...
public class JPanelBasic_v3 extends JPanel {
    ...
    final private SquareRootButtonHandler squareRootButtonHandler = new SquareRootButtonHandler();
    ...
    final private JButton buttonMemoryClear = new JButton("MC");
    ...
    final private MemorySwitchButtonHandler memorySwitchButtonHandler = new MemorySwitchButtonHandler();
    final private JButton buttonMemorySwitch = new JButton("MS");
    
    final private MemoryAddButtonHandler memoryAddButtonHandler = new MemoryAddButtonHandler();
    final private JButton buttonAddToMemory = new JButton("M+");

    final private MemorySubButtonHandler memorySubButtonHandler = new MemorySubButtonHandler();
    final private JButton buttonSubtractFromMemory = new JButton("M-");
    
    private int memoryPosition = 0;
    private String[] memoryValues = new String[]{"","","","","","","","","",""};
...
//    private boolean decimal = false;
	...
	public void setupPanel_v3() {
		LOGGER.info("Inside setupJPanelBasic_v3()");
        ...
        buttonSqrt.addActionListener(squareRootButtonHandler);
        ...
        buttonMemoryClear.addActionListener(memoryClearButtonHandler);
        ...
        buttonMemorySwitch.addActionListener(memorySwitchButtonHandler); 
        buttonMemorySwitch.setFont(Calculator_v3.font);
        buttonMemorySwitch.setPreferredSize(new Dimension(35, 35) );
        buttonMemorySwitch.setBorder(new LineBorder(Color.BLACK));
        buttonMemorySwitch.setEnabled(true);
        
        buttonAddToMemory.addActionListener(memoryAddButtonHandler);
        buttonAddToMemory.setFont(Calculator_v3.font);
        buttonAddToMemory.setPreferredSize(new Dimension(35, 35) );
        buttonAddToMemory.setBorder(new LineBorder(Color.BLACK));
        buttonAddToMemory.setEnabled(true);
        
        buttonSubtractFromMemory.addActionListener(memorySubButtonHandler);
        buttonSubtractFromMemory.setFont(Calculator_v3.font);
        buttonSubtractFromMemory.setPreferredSize(new Dimension(35, 35) );
        buttonSubtractFromMemory.setBorder(new LineBorder(Color.BLACK));
        buttonSubtractFromMemory.setEnabled(true);
        
        LOGGER.info("Finished. All buttons font set, size set, and border set.");
        LOGGER.info("Leaving setupJPanelBasic_v3()");
        // above this comment is all for the buttons
    } // end method setInitalStartMode()
    
    public void addComponentsToPanel_v3() {
    	LOGGER.info("Inside addComponentsToPanel_v3()");
    	constraints.fill = GridBagConstraints.BOTH;
        addComponent(calculator.getTextArea(), 0, 0, 5, 2);
        // Also add the action listener for each button
        constraints.fill = GridBagConstraints.HORIZONTAL;
        addComponent(buttonMemoryClear, 2, 0, 1, 1);
        
        constraints.fill = GridBagConstraints.HORIZONTAL;
        addComponent(buttonMR, 2, 1, 1, 1);
        
        constraints.fill = GridBagConstraints.HORIZONTAL;
        addComponent(buttonMemorySwitch, 2, 2, 1, 1);
        
        constraints.fill = GridBagConstraints.HORIZONTAL;
        addComponent(buttonAddToMemory, 2, 3, 1, 1);
        
        constraints.fill = GridBagConstraints.HORIZONTAL;
        addComponent(buttonSubtractFromMemory, 2, 4, 1, 1);
        ...
        LOGGER.info("Finished. All buttons added to the frame.");
        LOGGER.info("Leaving addComponentsToPanel_v3()");
    }
    ...
    public class SquareRootButtonHandler implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
        	LOGGER.info("SquareRootButtonHandler() started");
            ...
            if (!calculator.getTextArea().getText().equals("")) {
                if (Double.parseDouble(calculator.values[calculator.valuesPosition]) > 0.0) {
                    ...
                    //int intRep = (int)Math.sqrt(Double.parseDouble(values[valuesPosition])); // 4
...
//                    	calculator.textArea.setText("\n" + calculator.values[calculator.valuesPosition]);
                        ...
                        if (Integer.parseInt(calculator.values[calculator.valuesPosition]) < 0 ) {
                        	calculator.textarea = calculator.getTextArea().getText(); // values[2]
                            ...
                            LOGGER.info("values[0]: " + calculator.values[0]);
                        }
                    }
                    calculator.textArea.setText("\n" + calculator.values[calculator.valuesPosition] );
            ...
            LOGGER.info("Leaving SquareRootButtonHandler()");
            calculator.confirm();
        }
    }
    ...
    public class MemoryClearButtonHandler implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
    		...
    		LOGGER.info("button: " + e.getActionCommand()); // print out button confirmation
    		// clear memory by removing the last entry in the list
    		int theStringValueHere = calculator.memoryPosition;
    		if (memoryValues.length != 0 && calculator.memoryPosition > 0) {
    			boolean continueLoop = true;
    			do {
    				//theStringValueHere = calculator.memoryValues.size() - 1;
    				if (theStringValueHere >= 0 ) {
    					calculator.memoryValues[theStringValueHere--] = null;
    					calculator.memoryPosition--;
    				}
    				if (theStringValueHere != -1) { continueLoop = true; }
    				else 		              { continueLoop = false; }
    			} while (continueLoop);
    			int local = calculator.memoryPosition - 1;
    	    	if (local != -1) {
    	    		for (String memories : calculator.memoryValues) {
    	        		LOGGER.info("memoryValues.get("+local+"): " + memoryValues[local]);
    	        		local--;
    	        	}
    	    	} else {
    			...
    			LOGGER.info("No values in memory to remove");
    		}
            
            buttonMR.setEnabled(false);
            buttonMemoryClear.setEnabled(false);
            calculator.setMemoryValueAtPosition(memoryValues[memoryPosition], memoryPosition);
    ...
    // MemoryClearButtonHandler operates on this button: 
    // final private JButton buttonMemoryClear = new JButton("MR");
    ...
    // MemorySwitchButtonHandler operates on this button: 
    // final private JButton buttonMemorySwitch = new JButton("MS");
    public class MemorySwitchButtonHandler implements ActionListener {
    	@Override
        public void actionPerformed(ActionEvent e) {
    		LOGGER.info("MemorySwitchButtonHandler class started");
    		LOGGER.info("button: " + e.getActionCommand()); // print out button confirmation
    		// TODO: left separated for now. can combine if else into one if later!
    		if ( memoryValues[memoryPosition].equals("") && // if current memory position is blank, AND
    				(!calculator.addBool && !calculator.subBool && !calculator.mulBool && !calculator.divBool) ) { // no operators have been pressed
    			memoryValues[memoryPosition] = calculator.textarea.replaceAll("\n", "");
    			
    		} else if (!memoryValues[memoryPosition].equals("")) {
    			memoryValues[memoryPosition] = calculator.textarea.replaceAll("\n", "");
    		}
    		
    		if (!calculator.memorySwitchBool) { calculator.memorySwitchBool = true; }
    		else { calculator.memorySwitchBool = false; }
    ...
    // final private JButton buttonMemoryClear = new JButton("M+");
